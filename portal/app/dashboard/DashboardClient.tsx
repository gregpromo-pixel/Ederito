...
export default function DashboardClient({name,projects,contracts,invoices,tickets:initialTickets}:Props){
 ...
 const [tab,setTab]=useState<'projects'|'contracts'|'invoices'|'support'>('projects');
 ...
 async function submitTicket(event:FormEvent<HTMLFormElement>){
   ...
   const form=new FormData(event.currentTarget);const subject=String(form.get('subject')||'').trim();const description=String(form.get('description')||'').trim();const priority=String(form.get('priority')||'normal');
   ...
   if(error)setMessage(error.message);else{setTickets([data,...tickets]);setMessage(t.created);event.currentTarget.reset()}
   setBusy(false);
 }
 return <main className="portal shell">
  <header className="topbar">
    ...
    <nav className="navlinks">{(['projects','contracts','invoices','support'] as const).map(item=><button key={item} className={tab===item?'nav-active':''} onClick={()=>setTab(item)}>{t[item]}</button>)}</nav>
    <div className="top-actions"><div className="language-mini">{(['en','fr','es'] as Lang[]).map(x=><button key={x} onClick={()=>choose(x)} className={x===lang?'active':''}>{x.toUpperCase()}</button>)}</div><form action="/auth/signout" method="post"><button className="button secondary" type="submit">{t.signOut}</button></form></div>
  </header>
  ...
  <section className="stats"><article className="stat"><span>{t.activeProjects}</span><strong>{projects.length}</strong></article><article className="stat"><span>{t.contracts}</span><strong>{contracts.length}</strong></article><article className="stat"><span>{t.openInvoices}</span><strong>{openInvoices}</strong></article><article className="stat"><span>{t.openTickets}</span><strong>{openTickets}</strong></article></section>
  <section className="panel dashboard-panel">
   {tab==='projects'&&<><h2>{t.projects}</h2>{projects.length?projects.map(p=><div className="record" key={p.id}><strong>{p.name}</strong><span>{t.status}: {p.status}{p.target_launch_date?` · ${t.target}: ${p.target_launch_date}`:''}</span></div>):<div className="empty">{t.noProjects}</div>}</>}
   {tab==='contracts'&&<><h2>{t.contracts}</h2>{contracts.length?contracts.map(c=><div className="record" key={c.id}><strong>{c.title}</strong><span>{c.contract_number} · {t.status}: {c.status}</span></div>):<div className="empty">{t.noContracts}</div>}</>}
   {tab==='invoices'&&<><h2>{t.invoices}</h2>{invoices.length?invoices.map(i=><div className="record" key={i.id}><strong>{i.invoice_number}</strong><span>${(i.total_cents/100).toFixed(2)} · {t.status}: {i.status}{i.due_date?` · ${t.due}: ${i.due_date}`:''}</span></div>):<div className="empty">{t.noInvoices}</div>}</>}
   {tab==='support'&&<div className="support-layout"><div><h2>{t.support}</h2>{tickets.length?tickets.map(ticket=><div className="record" key={ticket.id}><strong>{ticket.subject}</strong><span>{ticket.priority} · {ticket.status}</span></div>):<div className="empty">{t.noTickets}</div>}</div><form className="form support-form" onSubmit={submitTicket}><h3>{t.createTicket}</h3><label className="field"><span>{t.subject}</span><input name="subject" required maxLength={160}/></label><label className="field"><span>{t.description}</span><textarea name="description" required minLength={10} maxLength={4000}/></label><label className="field"><span>{t.priority}</span><select name="priority"><option value="normal">{t.normal}</option><option value="high">{t.high}</option><option value="urgent">{t.urgent}</option></select></label>{message&&<div className="notice success">{message}</div>}<button className="button" disabled={busy}>{busy?'…':t.submit}</button></form></div>}
  </section>
 </main>
}