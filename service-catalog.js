(() => {
  ...
  const projectUrl = (service) => {
    ...
    return session().authenticated ? `${portal}${next}` : `${portal}/login?mode=register&next=${encodeURIComponent(next)}`;
  };

  function addStyles(){
    if(document.getElementById('ederito-catalog-styles'))return;
    const style=document.createElement('style');style.id='ederito-catalog-styles';style.textContent=`
  ...
  function cardActionText(){return session().authenticated?translated(['Open project intake','Ouvrir le formulaire','Abrir solicitud']):translated(['Create account to continue','Créer un compte pour continuer','Crear una cuenta para continuar']);}

  function refreshCardLinks(){
    document.querySelectorAll('.catalog-card[data-service-index]').forEach((card)=>{
      const service=services[Number(card.dataset.serviceIndex)];card.href=projectUrl(service);
      const label=card.querySelector('.catalog-card-action span');if(label)label.textContent=cardActionText();
    });
    refreshSecureEntry();
  }

  function installCatalog(){
    const section=document.getElementById('services');const wrap=section?.querySelector('.wrap');if(!wrap||wrap.querySelector('.catalog-grid'))return;
    ...
    services.forEach((service,index)=>{const card=document.createElement('a');card.className='catalog-card';card.dataset.serviceIndex=String(index);card.href=projectUrl(service);card.innerHTML=`<div class="catalog-card-top"><span>${service.number}</span><span>${translated(['Secure portal','Portail sécurisé','Portal seguro'])}</span></div><h3>${translated(service.title)}</h3><p>${translated(service.text)}</p><div class="catalog-card-action"><span>${cardActionText()}</span><b>+</b></div>`;grid.appendChild(card)});
    const domain=document.createElement('article');domain.className='catalog-card coming-soon';domain.innerHTML=`<div class="catalog-card-top"><span>11</span><span>${translated(['Coming soon','Bientôt disponible','Próximamente'])}</span></div><h3>${translated(['Buy a domain','Acheter un nom de domaine','Comprar un dominio'])}</h3><p>${translated(['Search, purchase and manage domains directly through Ederito.','Recherchez, achetez et gérez vos domaines directement avec Ederito.','Busca, compra y administra dominios directamente con Ederito.'])}</p><div class="catalog-card-action"><span>${translated(['Domain marketplace coming soon','Marché de domaines bientôt disponible','Mercado de dominios próximamente'])}</span><b>·</b></div>`;grid.appendChild(domain);
  }

  function refreshSecureEntry(){
    const box=document.querySelector('.secure-project-entry');if(!box)return;const loggedIn=session().authenticated;
    box.innerHTML=loggedIn?`<p class="eyebrow">${translated(['Secure project intake','Demande de projet sécurisée','Solicitud de proyecto segura'])}</p><h3>${translated(['Welcome back. Start your next project.','Bon retour. Démarrez votre prochain projet.','Bienvenido de nuevo. Inicia tu próximo proyecto.'])}</h3><p>${translated(['Your dashboard keeps requests, messages, proposals, agreements, invoices and updates together.','Votre tableau de bord rassemble demandes, messages, propositions, contrats, factures et suivis.','Tu panel reúne solicitudes, mensajes, propuestas, contratos, facturas y actualizaciones.'])}</p><div class="secure-entry-actions"><a class="btn btn-gold" href="${portal}/dashboard">${translated(['Open dashboard','Ouvrir le tableau de bord','Abrir panel'])}</a><a class="btn btn-dark" href="#services">${translated(['Choose a service','Choisir un service','Elegir un servicio'])}</a></div>`:`<p class="eyebrow">${translated(['Secure project intake','Demande de projet sécurisée','Solicitud de proyecto segura'])}</p><h3>${translated(['Create an account before starting a project.','Créez un compte avant de démarrer un projet.','Crea una cuenta antes de iniciar un proyecto.'])}</h3><p>${translated(['Your account keeps every project document and update together.','Votre compte centralise tous les documents et suivis du projet.','Tu cuenta reúne todos los documentos y actualizaciones del proyecto.'])}</p><div class="secure-entry-actions"><a class="btn btn-gold" href="#services">${translated(['Choose a project','Choisir un projet','Elegir un proyecto'])}</a><a class="btn btn-dark" href="${portal}/login">${translated(['Client login','Connexion client','Acceso cliente'])}</a></div>`;
  }

  function secureContactSection(){const form=document.getElementById('contactForm');if(!form)return;form.className='secure-project-entry reveal visible';form.removeAttribute('id');refreshSecureEntry();}
  function updatePrimaryLinks(){document.querySelectorAll('a[href="#contact"]').forEach(link=>link.setAttribute('href','#services'))}
  function rerenderCatalog(){const wrap=document.getElementById('services')?.querySelector('.wrap');if(wrap){wrap.innerHTML='';installCatalog()}refreshSecureEntry();refreshCardLinks()}
  function initialize(){addStyles();installCatalog();secureContactSection();updatePrimaryLinks();window.addEventListener('ederito:session',refreshCardLinks);window.addEventListener('ederito:language',rerenderCatalog);setTimeout(refreshCardLinks,0)}
